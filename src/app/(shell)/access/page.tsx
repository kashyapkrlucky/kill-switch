...
import { Key, Copy, Check, Code, Terminal, Shield, Clock } from "lucide-react";
...
export default function ApiAccess() {
  ...
  const { loading, getTokens, tokens, generateToken } = useTokenStore();
  ...
  useEffect(() => {
    if (selectedProject) {
      getTokens(selectedProject);
    }
  }, [selectedProject, getTokens]);
        ...
        <div className="w-full lg:w-2/3 space-y-4">
          <div className="bg-slate-800/50 backdrop-blur-sm border border-slate-700 rounded-lg p-6">
            ...
            <div className="flex flex-row items-end gap-4">
              ...
              <Button
                onClick={() => generateToken(selectedProject)}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div className="bg-slate-900/50 rounded-lg p-4">
                      <div className="text-2xl font-bold text-emerald-400">
                        {token?.usage.requests.toLocaleString()}
                      </div>
                    ...
                    <div className="bg-slate-900/50 rounded-lg p-4">
                      <div className="text-2xl font-bold text-blue-400">
                        {token?.usage.limit.toLocaleString()}
                      </div>
                      <div className="text-xs text-gray-400 mt-1">
                        Monthly Limit
                      </div>
                    ...
                    <div className="bg-slate-900/50 rounded-lg p-4">
                      <div className="text-2xl font-bold text-orange-400">
                        {Math.round(
                          (token?.usage.requests / token?.usage.limit) * 100
                        )}
                        %
                      </div>
          ...
          <div className="space-y-3">
            ...
            <div className="bg-slate-900/50 rounded-lg p-3">
              ...
              <code className="block text-xs text-gray-300 font-mono whitespace-pre-wrap">
                {`curl -X GET ${process.env.NEXT_PUBLIC_API_URL}/access/flags \\
  ...
  -H "Content-Type: application/json"`}
            ...
            <div className="bg-slate-900/50 rounded-lg p-3">
              ...
              <code className="block text-xs text-gray-300 font-mono whitespace-pre-wrap">
                {`const response = await fetch('${process.env.NEXT_PUBLIC_API_URL}/access/flags', {
  headers: {
    'Authorization': 'Bearer YOUR_TOKEN',
