...
export type FlagEnvironmentType =
  (typeof FlagEnvironment)[keyof typeof FlagEnvironment];

export type FlagEnvironmentState = Record<
  FlagEnvironmentType,
  {
    status: FlagStatusType;
    updatedAt?: string;
...
export const FLAG_ENVIRONMENTS: {
  key: FlagEnvironmentType;
  label: string;
  shortLabel: string;
}[] = [
  { key: FlagEnvironment.DEVELOPMENT, label: "Development", shortLabel: "Dev" },
  { key: FlagEnvironment.STAGING, label: "Staging", shortLabel: "Stage" },
  { key: FlagEnvironment.PRODUCTION, label: "Production", shortLabel: "Prod" },
];
