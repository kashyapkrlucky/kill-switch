...
export default function FlagList({ flags, isDashboard = false }: { flags: IFlag[]; isDashboard?: boolean }) {
  ...
  return (
    <>
      <div className={`${isDashboard ? "" : "bg-slate-800/50 backdrop-blur-sm border border-slate-700 rounded-lg overflow-hidden"}`}>
        {!isDashboard && (
        ...
        <div className={`${isDashboard ? "" : "divide-y divide-slate-700"}`}>
          {flags.length > 0 ? (
            <div className={`${isDashboard ? "divide-y divide-slate-700" : ""}`}>
              {flags.map((flag) => (
                <div
                ...
                >
                  ...
                  <div className="col-span-2 flex items-center gap-2">
                    ...
                    <span className="text-xs text-gray-400">
                      {flag.status === FlagStatus.ACTIVE ? "Active" : "Inactive"}
                    </span>
                  </div>
